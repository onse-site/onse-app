...
import { DeleteOffice } from "../../ecommerce/ApiHelper.ts";
                ...
                <TableRow key={office.id}>
                  <TableCell className="px-5 py-4 sm:px-6 text-start">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <span className="block w-30 font-medium text-gray-800 text-theme-sm dark:text-white/90">
                          {office.name}
                        </span>
                  ...
                  <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                    <button
                      ...
                      id={office.id}
                      onClick={async (e) => {
                        const message = await DeleteOffice(
                          (e.target as HTMLButtonElement).id
                        );
                        showNotification(
                          message,
                          message === "Error deleting office" ? "red" : "green"
                        );
                      }}
                    >
                      Delete
